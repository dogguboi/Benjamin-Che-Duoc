package com.mycompany.e3_s8_benjamin_che;
...
public class E3_S8_Benjamin_Che {
    ...
    private final List<List<Boolean>> asientos = new ArrayList<>(4);
        ...
        public void run() {
            ...
            while (bloqueados < 2) {
                ...
                if (asientos.get(zona).get(asiento)) {
                    ...
                    // Registrar como venta normal
                    double precio = getPrecio(zona);
                    int orderId = nextOrderId++;
                    Orden nuevaOrden = new Orden(orderId, "Venta Automática");
    ...
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) {
            E3_S8_Benjamin_Che teatro = new E3_S8_Benjamin_Che();
    ...
    private void comprarEntrada(Scanner sc) {
        ...
        double descuento = calcularDescuento(edad, cantidad);
    ...
    private double calcularDescuento(int edad, int cantidad) {
        double descuento = 0;
        if (edad < 20) descuento += 0.10;
    ...
    private void generarBoleta(Orden orden, int tipo, List<Integer> asientosSel, double total) {
        ...
        String boleta = String.format(
            "\n=== BOLETA ORDEN #%d ===\nCliente: %s\nZona: %s\nAsientos: %s\nTotal: $%,.2f\n",
            orden.id, orden.cliente, zona, asientosSel, total
        );
    ...
    private void buscarOrden(Scanner sc) {
        ...
        for (Entrada e : orden.entradas) {
            String desc = e.edad < 20 ? "[Estudiante]" : e.edad > 60 ? "[Adulto Mayor]" : "";
